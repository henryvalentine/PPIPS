...
{
    app.register.directive('ngPrNtfs', function ($compile)
    {
        return function ($scope, ngPrNtfs)
        {
            var tableOptions = {};
            tableOptions.sourceUrl = "/Notification/GetProcessingNotifications";
            ...
            var ttc = adminNotificationTableManager($scope, $compile, ngPrNtfs, tableOptions);
    ...
    app.register.controller('processingNotificationsController', ['ngDialog', '$scope', '$rootScope', '$routeParams', 'userNotificationService', '$upload', 'fileReader', '$http', '$location','$window',
    function (ngDialog, $scope, $rootScope, $routeParams, userNotificationService, $upload, fileReader, $http, $location,$window)
    {
        ...
        {
            $rootScope.setPageTitle('Notifications in Processing|DPR-PPIPS');
            $scope.viewApp = false;
            $scope.notification =
             {
                 'NotificationId': '',
