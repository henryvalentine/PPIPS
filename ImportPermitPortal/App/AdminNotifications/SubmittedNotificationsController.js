...
{
    app.register.directive('ngSNtfs', function ($compile)
    {
        return function ($scope, ngSNtfs)
        {
            var tableOptions = {};
            tableOptions.sourceUrl = "/Notification/GetSubmittedNotifications";
            ...
            var ttc = adminNotificationTableManager($scope, $compile, ngSNtfs, tableOptions);
    ...
    app.register.controller('submittedNotificationsController', ['ngDialog', '$scope', '$rootScope', '$routeParams', 'userNotificationService', '$upload', 'fileReader', '$http', '$location', '$window',
    function (ngDialog, $scope, $rootScope, $routeParams, userNotificationService, $upload, fileReader, $http, $location,$window)
    {

        $scope.initializeApp = function ()
        {
            $rootScope.setPageTitle('Submitted Notifications|DPR-PPIPS');
            $scope.viewApp = false;
            $scope.notification =
             {
                 'NotificationId': '',
