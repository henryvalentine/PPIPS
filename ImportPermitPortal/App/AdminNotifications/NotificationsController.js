...
{
    app.register.directive('ngNNtfs', function ($compile)
    {
        return function ($scope, ngNNtfs)
        {
            var tableOptions = {};
            ...
            var ttc = adminNotificationTableManager($scope, $compile, ngNNtfs, tableOptions);
    ...
    app.register.controller('notificationsController', ['ngDialog', '$scope', '$rootScope', '$routeParams', 'userNotificationService', '$upload', 'fileReader', '$http', '$location','$window',
    function (ngDialog, $scope, $rootScope, $routeParams, userNotificationService, $upload, fileReader, $http, $location,$window)
    {
        ...
        $scope.getReqsCompleted = function (response) {
            ...
            if ($scope.notification.Status > 2)
            {
