...
{
    public class PermitBulkToJsonController : Controller
    {
        ...
        private List<long> ReadExcelData(string filePath, string sheetName, ref List<long> errorList, ref string msg, ref List<PermitObject> permits)
        {
            if (filePath.Length < 3 || new FileInfo(filePath).Exists == false) 
            {
                msg = "Invalid Excel File Format";
                errorList = new List<long>();
                permits = new List<PermitObject>();
            ...
            var selectString = @"SELECT [Company],[Reference_Number],[Issue_Date],[Expiry_Date] FROM [" + sheetName + "$]";
            ...
            try
            {
                ...
                var parseList = new List<PermitObject>();
