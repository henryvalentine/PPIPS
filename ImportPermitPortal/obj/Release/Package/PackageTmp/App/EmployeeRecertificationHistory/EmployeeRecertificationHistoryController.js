...
define(['application-configuration', 'employeeRecertificationHistoryService', 'ngDialog', 'angularFileUpload', 'fileReader'], function (app) {
    app.register.directive('ngRecertHis', function ($compile) {
        return function ($scope, ngRecertHis) {
            var tableOptions = {};
            tableOptions.sourceUrl = "/EmployeeHistory/GetPreviousRecertificationJobs";
            tableOptions.itemId = 'Id';
            tableOptions.columnHeaders = ['ReferenceCode','CompanyName', 'AssignedTimeStr', 'ActualDeliveryDateTimeStr', 'Remarks'];
          
            var ttc = adminApplicationtableManager($scope, $compile, ngRecertHis, tableOptions);
            ttc.removeAttr('width').attr('width', 'auto');
            $scope.jtable = ttc;
        };
    });

    app.register.controller('employeeRecertificationHistoryController', ['ngDialog', '$scope', '$rootScope', '$routeParams', '$upload', 'fileReader', '$http', 'employeeRecertificationHistoryService', '$route', '$window', '$location',
    function (ngDialog, $scope, $rootScope, $routeParams, $upload, fileReader, $http, employeeRecertificationHistoryService, $route, $window, $location) {
        ...
        $scope.getAppView = function (appId) {
            ...
            $scope.initializeApp();
            $http({ method: 'GET', url: '/EmployeeHistory/GetRecertification?id=' + appId }).success(function (response) {
        ...
        $scope.initializeController = function () {
            $rootScope.setPageTitle('Recertification Histories');
            $scope.viewApp = false;
            $scope.application =
            {
                'ApplicationId': '', 'Id': '', 'ReferenceCode': '', 'DerivedTotalQUantity': '',
