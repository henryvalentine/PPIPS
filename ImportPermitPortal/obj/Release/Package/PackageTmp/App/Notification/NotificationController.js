...
define(['application-configuration', 'notificationService','ngDialog', 'angularFileUpload', 'fileReader'], function (app)
{
    
    app.register.controller('notificationController', ['ngDialog', '$scope', '$rootScope', '$routeParams', 'notificationService', '$location',
    function (ngDialog, $scope, $rootScope, $routeParams, notificationService, $location)
    {
        var xcvb = new Date();
        ...
        {
            notificationService.getGenericList($scope.getListsCompleted);
        };
        ...
        {
            //if (data == null || data.Countries.length < 1)
            //{
            ...
            $scope.permits = data.PermitObjects;
        ...
        {
            if ($scope.notification.PermitObject == null || $scope.notification.PermitObject.Id < 1)
            {
                alert('Please select a Permit.');
                return;
            }

            notificationService.getApplicationByPermitValue($scope.notification.PermitObject.PermitValue, $scope.serchAppByPermitValueCompleted);
        };
            ...
            $scope.notification =
             {
                 ...
                 'QuantityToDischarge': '',
                 'QuantityOnVessel': '',
                 'CargoInformationTypeId' : 0,
                 'ArrivalDate': '',
                 'DischargeDate' : '',
                 'AmountDue' : '',
                 'DateCreated': '',
                 'Port': { 'Id': '', 'Name': '-- Select Port--', 'CountryId': '' },
                 'Product': { 'ProductId': '', 'Code': '-- Select Product--', 'Name': '', 'Availability': '' },
                 'Depot': { 'Id': '', 'Name': '-- Select Disharge Depot--'},
                 'Country': { 'Id': '', 'Name': '-- Select Country --', 'CountryCode': '', 'RegionId': '' },
                 'ClassObject': { 'Id': '', 'Name': '-- Select Notification Category --' },
                 'PermitObject': { 'Id': '', 'PermitValue': '-- Select Permit --' },
                 'FeeObjects' : []  
             };
        ...
        $scope.getValWithReqs = function ()
        {
            ...
            notificationService.getAmountDueWithReqs(notificationProps, $scope.getValWithReqsCompleted);
        };
        ...
        $scope.processNotification = function ()
        {
            ...
            var prCode = '';
            var importApplicationId = $scope.permitApplication.ApplicationId;
            angular.forEach($scope.permitApplication.ApplicationItemObjects, function (z, t)
            {
                if (z.ProductId === $scope.notification.ApplicationItem.ProductObject.ProductId)
                {
                    appQuantity = z.EstimatedQuantity;
                    $scope.impObject = z;
                    importedQuantity = z.QuantityImported;
                }
            });
            ...
            if (parseInt($scope.notification.ClassObject.Id) === 1)
            {
                ...
                if ($scope.notification.QuantityToDischarge > appQuantity) 
                {
            ...
            if ($scope.notification.ClassObject == null || $scope.notification.ClassObject.Id < 1)
            {
             ...
             var notification =
             {
                 'NotificationId': $scope.notification.NotificationId,
                 'ReferenceCode': $scope.notification.ReferenceCode,
                 'ClassificationId': $scope.notification.ClassObject.Id,
                 'ApplicationQuantity': appQuantity,
                 'QuantityImported': importedQuantity,
           ...
           if ($scope.add === true) {
               notificationService.addNotification(notification, $scope.processNotificationCompleted);
           }
           if ($scope.edit === true) {
               notificationService.editNotification(notification, $scope.processNotificationCompleted);
           }
        };
