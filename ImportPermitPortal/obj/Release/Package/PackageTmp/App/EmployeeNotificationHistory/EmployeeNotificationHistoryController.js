...
define(['application-configuration', 'employeeNotificationHistoryService', 'ngDialog', 'angularFileUpload', 'fileReader'], function (app) {
    app.register.directive('ngNotHis', function ($compile) {
        return function ($scope, ngNotHis) {
            var tableOptions = {};
            tableOptions.sourceUrl = "/EmployeeHistory/GetPreviousNotificationJobs";
            tableOptions.itemId = 'Id';
            tableOptions.columnHeaders = ['ReferenceCode','CompanyName', 'AssignedTimeStr', 'ActualDeliveryDateTimeStr', 'Remarks'];
          
            var ttc = adminApplicationtableManager($scope, $compile, ngNotHis, tableOptions);
            ttc.removeAttr('width').attr('width', 'auto');
            $scope.jtable = ttc;
        };
    });

    app.register.controller('employeeNotificationHistoryController', ['ngDialog', '$scope', '$rootScope', '$routeParams', '$upload', 'fileReader', '$http', 'employeeNotificationHistoryService','$route','$window','$location',
    function (ngDialog, $scope, $rootScope, $routeParams, $upload, fileReader, $http, employeeNotificationHistoryService,$route,$window, $location) {
        ...
        $scope.getAppView = function (appId) {
            ...
            $scope.initializeApp();
            $http({ method: 'GET', url: '/EmployeeHistory/GetNotification?id=' + appId }).success(function (response) {
        ...
        $scope.initializeController = function () {
            $rootScope.setPageTitle('Notification Histories');
            $scope.viewApp = false;
            $scope.application =
            {
                'ApplicationId': '', 'Id': '', 'ReferenceCode': '', 'DerivedTotalQUantity': '',
